...
CREATE TABLE users (
    ...
    role CHAR(45) NOT NULL CHECK (role IN ('ROLE_USER','ROLE_ADMIN')),
	PRIMARY KEY(uid)
);
...
CREATE TABLE News_Metadata_Tags (
    ...
    PRIMARY KEY(nmid, tid)
);

CREATE TABLE News_Metadata_Users (
	nmid INT(11) NOT NULL,
    uid INT(11) NOT NULL,
    stars TINYINT NOT NULL,
    PRIMARY KEY(nmid, uid)
);
...
ALTER TABLE News_Metadata_Users ADD CONSTRAINT fk_news_meta_users_nmid 
FOREIGN KEY (nmid) REFERENCES News_Metadata(nmid) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE News_Metadata_Users ADD CONSTRAINT fk_news_meta_users_uid 
